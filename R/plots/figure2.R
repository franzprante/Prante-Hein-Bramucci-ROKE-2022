...
p_gr_Y <- plot_ly() %>% 
  ...
  layout(xaxis = list(range = c(-10, 1500),
                      tickfont = list(size = 8)),
         yaxis = list(range = c(0.015, 0.018),
                      tickfont = list(size = 8)),
         annotations = gr_Y,
         showlegend = TRUE,
         hovermode = "compare")
...
p_G <- plot_ly() %>% 
  add_trace(x = 0:(nrow(d2)-1),
            y = round(d2$G_Y, digits = 2),
            type = "scatter",
            mode = "lines",
            line = list(color = "black"),
            name = "G/Y") %>% 
  add_trace(x = 0:(nrow(d2)-1),
            y = round(d2$G_Yx, digits = 2),
            type = "scatter",
            mode = "lines",
            line = list(color = "red",
                        dash = "dash"),
